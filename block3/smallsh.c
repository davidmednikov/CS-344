...
char* expand$$ToPid(char* string) {
    if (strstr(string, "$$") != NULL) {
        ...
        char* before$$ = strtok(string, "$$");
        char* after$$ = strtok(NULL, "$$");

        // create string to hold
        char returnString[2100];

        // if text before $$, concatenate token and pid together and copy to return string
        if (before$$ != NULL) {
            strcpy(returnString, strcat(before$$, pidString));
        } else {
            // else $$ is at start of string, copy to return string
            strcpy(returnString, pidString);
        }

        // if text after $$, concatenate to return string
        if (after$$ != NULL) {
            strcat(returnString, after$$);
        }
...
/*
** printKilled Function
** ----------------------------------------------------
** function to print info about the signal that terminated the most recent process
** params: none
** returns: none
** ----------------------------------------------------
*/
void printKilled() {
    ...
    // get pid of killed process and copy status code to status
    pid_t killedProcess = waitpid(-1, &status, WNOHANG);

    // if pid > 0, that means there was a process that just terminated
    if (killedProcess > 0) {
        // if process was killed by signal, write info about the signal
        if (WIFSIGNALED(status)) {
            char message[100];
            write(STDOUT_FILENO, "terminated by signal %d\n", WTERMSIG(status));
        }
    }
}
